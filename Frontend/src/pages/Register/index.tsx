...
export default function Register() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[linear-gradient(to_right,var(--color-brand-100)_0%,var(--color-brand-100)_50%,var(--color-brand-200)_50%,var(--color-brand-200)_100%)] dark:bg-[linear-gradient(to_right,var(--color-slate-900)_0%,var(--color-slate-900)_50%,var(--color-slate-950)_50%,var(--color-slate-950)_100%)] p-6 transition-colors duration-300">
      <div className="min-h-screen md:min-h-[70vh] w-full max-w-5xl flex flex-col md:flex-row bg-white dark:bg-slate-900 rounded-3xl shadow-2xl overflow-hidden border border-transparent dark:border-slate-800">
        <div className="p-8 md:w-[49%]">
          ...
          <form onSubmit={register} className="w-full">
            <div className="mb-5">
              <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Bem-vindo!</h1>
              <p className="mt-2 text-gray-600 dark:text-gray-400">Estamos felizes por você estar aqui. Sua escolha de ensinar ou aprender já demonstra coragem para crescer.</p>
            </div>


            {error && (
              <Alert variant="destructive" className="mb-4 border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-950/50 text-red-700 dark:text-red-200">
                <AlertDescription>{error}</AlertDescription>
              </Alert>
            )}

            <div className="mb-4">
              <label className="block text-sm text-gray-700 dark:text-gray-300" htmlFor="email">Email</label>
              <input
                id="email"
                type="email"
                value={email}
            ...
            <div className="mt-4 text-center text-sm">
              ...
              <button type="button" onClick={() => navigate("/login")} className="text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 cursor-pointer">
                Entrar
              </button>
            </div>
          </form>
